    ...
    <el-dialog title="详情" v-model="dialogVisible" width="620px" :before-close="handleClose">
      <div v-if="currentRow">
        <el-descriptions :column="2" border class="user-detail-descriptions">
          ...
          <el-descriptions-item label="值类型">{{ valueTypeTextMap[currentRow.value_type] }}</el-descriptions-item>
    ...
    <el-dialog title="添加" v-model="addDialogVisible" width="620px" :before-close="() => (addDialogVisible = false)">
      <el-form :model="addForm" label-width="100px">
        ...
        <el-form-item label="值类型">
          <el-select v-model="addForm.value_type" placeholder="请选择值类型" filterable>
...
export default defineComponent({
  ...
  methods: {
    ...
    submitEdit() {
      updateConfig(this.editForm.id, this.editForm).then(() => {
    ...
    submitAdd() {
      createConfig(this.addForm).then(() => {
